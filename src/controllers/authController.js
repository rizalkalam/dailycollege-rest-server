...
const verifyAndRegisterUser = async (req, res) => {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      if (
        data &&
        parseInt(data.verificationCode) === parseInt(verificationCode)
      ) {
        userData = data;
        break;
      }
    ...
    if (!userData) {
      return res
        .status(400)
        .json({ message: "Kode verifikasi anda salah/expired" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Verifikasi sukses data berhasil terdaftar" });
  } catch (error) {
...
const get_token = async (req, res) => {
  try {
    ...
    if (!sessionData) {
      return res
        .status(401)
        .json({ message: "Session tidak valid atau kedaluwarsa" });
    }
